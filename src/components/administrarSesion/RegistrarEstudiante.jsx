...
function RegistrarEstudiante() {
    ...
    const handleSubmit = async (event) => {
        ...
        const regexNombres = /^\S+(\s\S+){1}$/;
        if (!regexNombres.test(usuario.nombres.trim())) {
            showSnackbar('Por favor, ingrese sus dos nombres.');
            return;
        }

        const regexApellidos = /^\S+(\s\S+){1}$/;
        if (!regexApellidos.test(usuario.apellidos.trim())) {
            showSnackbar('Por favor, ingrese sus dos apellidos.');
            return;
        }

        if (!validateEmail(usuario.email)) {
            showSnackbar('Por favor, ingresa un correo electrónico válido.');
            return;
        }
        ...
        try {
            ...
            if (en === 1) {
                ...
                setRegistroExitoso(true);
                handleLimpiarCampos();
                showSnackbar('Registro exitoso. ¡Bienvenido!', 'success');
            } else {
                console.error('Error en el registro:', m);
                showSnackbar(m);
            }
            ...
            console.error('Error al enviar los datos al backend:', error);
            showSnackbar('Error al registrar. Por favor, inténtelo de nuevo.');
        }
    };
